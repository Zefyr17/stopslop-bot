...
async function handleVoteButton(interaction: ButtonInteraction, guildId: string) {
  try {
    ...
    const voteType = interaction.customId.startsWith('upvote_') ? VoteType.UP : VoteType.DOWN;
    ...
    if (freshPost?.status === PostStatus.PENDING) {
      ...
      if (weightedVoteCounts.weightedDownvotes >= config.downvoteThreshold) {
        // Community rejected — flag for admin review
        ...
        // Community approved — auto-shortlist immediately
        await postService.updateStatus(post.id, PostStatus.SHORTLISTED);
        statusChanged = true;

        await modLogService.log(guildId, ModLogEventType.POST_SHORTLISTED_AUTO, flagData);
    ...
    const isDecided = statusChanged;
    const buttonRow = new ActionRowBuilder<ButtonBuilder>().addComponents(
      new ButtonBuilder().setCustomId(`upvote_${post.id}`).setLabel('Yes').setStyle(ButtonStyle.Primary).setDisabled(isDecided),
      new ButtonBuilder().setCustomId(`downvote_${post.id}`).setLabel('No').setStyle(ButtonStyle.Secondary).setDisabled(isDecided)
    );
...
async function handleSlashCommand(interaction: ChatInputCommandInteraction, guildId: string) {
  ...
  try {
    ...
    if (commandName === 'config') {
      ...
      if (subcommand === 'set-ticket-channel') {
        const channel = interaction.options.getChannel('channel', true);
        if (channel?.type !== ChannelType.GuildText) {
          await interaction.reply({ content: '❌ Channel must be a text channel.', ephemeral: true });
          return;
        ...
        await interaction.reply({ content: `✅ Ticket channel set to <#${channel.id}>`, ephemeral: true });
        return;
      }
